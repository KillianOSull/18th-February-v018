...
console.log('Start of programme')						//prints on console						//
mqttClient.on('message', messageEventHandler);					//handeling messages from mqtt broker				//
  	...
  	console.log('Device: [', deviceName , '] is within range')		//prints on command line					//
										//								//
  	await adapter.stopDiscovery() 						//stops looking for devices					//
  	console.log("Stopping Discovery")					//prints on command line					//
  										//								//
	...
	console.log("services are " + services)					//prints on command line					//
  										//								//
//********************************************************BLE OPTIONS AND CONNECTING TO A DEVICE************************************************//
	...
	if (services.includes(codeServiceOfInterestUuid)) { //code Services
  	...
  	if (services.includes(buttonServiceOfInterestUuid)) { //Button Services
	 	...
	 	buttonAChar.on('valuechanged', async BtnA => {
	 		...
	 		console.log('interget counter from NodeJS: ' + unlock_counter_val_int)
	 		...
	 		cahnge_unlock_code = unlock_code_val.toString('hex')
	 		console.log(unlock_code_val);
	 		process.stdout.write('The Unlock Code from NodeJS is: ') //code sent is as ABCDEFGH... code recieved as GHEFCDAB because of big and little endians 
	 		...
	 		*/
	 		...
	 		const new_unlock_code_val = cahnge_unlock_code.charAt(6) + cahnge_unlock_code.charAt(7) + cahnge_unlock_code.charAt(4) + cahnge_unlock_code.charAt(5) + cahnge_unlock_code.charAt(2) + cahnge_unlock_code.charAt(3) + cahnge_unlock_code.charAt(0) + cahnge_unlock_code.charAt(1);
			...
			try {
			  ...
			  if (unlock_match == 1) { console.log ('ITS A MATCH') }
			  if (unlock_match == 0) { console.log ('NOT A MATCH') }
			} catch (err) {
			  console.error(err)
			}
	...
	if (services.includes(buttonServiceOfInterestUuid)) {
		...
		buttonBChar.on('valuechanged', async BtnB => {
	 		...
	 		console.log('interget counter: ' + lock_counter_val_int)
	 		...
	 		process.stdout.write('The Lock Code is: ') //code sent is as ABCDEFGH... code recieved as GHEFCDAB because of big and little endians 
			...
			try {
			  await new Promise(resolve => setTimeout(resolve, 2000)); //pauses the programme
			  const lock_match = fs.readFileSync('lock_match.txt', 'utf8')
			  if (lock_match == 1) { console.log ('ITS A MATCH') }
			  if (lock_match == 0) { console.log ('NOT A MATCH') }
			} catch (err) {
			  console.error(err)
			}
